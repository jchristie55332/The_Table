  ...
  app.controller('mapController', ["$scope", "$http", function($scope, $http) {
    ...
    $http.get("/restaurants.json").success(function(data){
      navigator.geolocation.getCurrentPosition(function(position){
        $scope.$apply(function() {
          ...
          for (var i = 0; i < data.length; i++) {
            if ($scope.food_styles.indexOf(data[i].food_style) == -1) {
              $scope.food_styles.push(data[i].food_style);
            }
            var lat1 = data[i].lat;
            var lon1 = data[i].lng;
      ...
      $scope.food_styles = $scope.food_styles.filter(function(item, i, ar){ return ar.indexOf(item) === i; })
    });
